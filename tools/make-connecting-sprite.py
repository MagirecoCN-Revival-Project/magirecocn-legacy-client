...
# 描边取**与字芯同一族的深色**，而不是原版那圈洋红：文字自带「深色字芯 + 白描边」，
# 丘比用白填充 + 深描边，两者就是同一套白/深两色语言的正反面，看起来是一个整体。
# 原版之所以用洋红，是因为它的英文字也带洋红辉光；我们的中文字没有，再上洋红就
# 成了画面里唯一一处第三色，反而扎眼。
STROKE_RGB   = (27, 25, 23)
STROKE_WIDTH = 2                # 丘比用。4px 在白底上像给它镶了道黑框，2px 刚够把轮廓兜住
STROKE_FADE  = 1                # 最外这几圈渐隐；再往内一律实心
...
def main():
    ...
    tb = text.getbbox()
    text = text.crop(tb)
    # 文字**不额外描边**：国服这套字自带「深色字芯 + 白色描边」，本身就是
    # 两色对比结构——白底靠字芯、深底靠白边，两种背景都立得住。再往外套一圈，
    # 汉字笔画密、字距只有 2px 左右，实测 2px 会把字黏成一片、1px 也把字撑肥，
    # 连三个点都各自套上小方框。丘比不一样：它是纯白剪影，白底上必然消失，
    # 描边对它是必需品而不是装饰。
