...
**碰到一句没汉化，先定位它属于上表哪一行，再动手。** 判据不能靠猜——补表和改前端
是两个方向完全不同的修法，猜错整批活白干。native 那一行有现成的判定手段：开
`logI18nMiss` 跑一遍，日志里出现该串就说明它流经 native 标签（补 `engine_i18n.tsv`
即可，且热重载生效），没出现就说明得往上找前端热更包或服务端。做法见「调试开关
目录」的[「这句为什么没汉化」怎么查](#这句为什么没汉化怎么查logi18nmiss)。
...
| 前缀 | 干什么 | 什么时候用 |
|---|---|---|
| `skipXxx` / `noXxx` | 跳过启动链上的某一步 / 关掉某个 native 改动 | 二分定位「是哪一步把游戏搞挂的」 |
| `failXxx` / `slowXxx` | 故障注入 | 验证错误处理路径本身——退避重试、换线、事务回滚、慢网询问框。这些平时**只有网络真的烂掉才跑得到**，没有注入手段就等于从没测过 |
| `logXxx` | **只记录，不改行为** | 把平时不打日志的东西打出来。加它的场合是「日志里根本没有能回答这个问题的信息」，而不是「日志太少」 |
...
### 「这句为什么没汉化」怎么查（`logI18nMiss`）

文本钩子原本**只记命中、不记未命中**：

```cpp
if (n <= 10 || n % 100 == 0)
    LOGI("[i18n] 替换 #%llu: %.40s", ...);   // 只有命中才有这行
// 未命中直接转发，一声不吭
```

这让「某句为什么还是日文」变成一个**问不出答案的问题**——串不在表里时，它有没有
流经 native 标签，日志长得一模一样。而这两种情况的修法完全相反：走 native 就补
`engine_i18n.tsv`，不走就得去前端热更包或服务端那一层。猜错方向，整批活白干。

2026-08-08 战斗结束那句「カーテンコールで終いやな」就卡在这上面。`logI18nMiss`
...
输出**就是 tsv 的行格式**，换行/制表按同一套规则转义，所以抓下来填上译文就能用，
不必手工誊写。追加到设备上那份表可以就地验证——表是热重载的（3 秒查一次 mtime），
不用重启也不用重出包；但那只是**运行时副本**，译文要落地得提到补丁仓库
`magireco-cn-patch` 的 `madomagi/engine_i18n.tsv`，否则下次台词包更新就盖掉了。

> ⚠ 每行以 `#` 打头，**这是故意的**。表里「译文为空」的语义不是「还没翻」，而是
> **删除该串**（拼接式文案调语序用的）。所以未填译文的骨架行不是惰性的——不加 `#`
> 直接追加，这些串会当场从界面上消失，而且是在没人改过译文的情况下悄悄发生。
> 加了 `#` 追加就是纯粹的空操作，翻一条放开一条。

| 开关 | 记什么 | 代价 |
|---|---|---|
| `logI18nMiss` | 只记**含假名**（U+3040–U+30FF）的未命中串 | 漏掉纯汉字的日文短语（如「全体攻撃」） |
| `logI18nMissAll` | 不筛内容，含英文/数字 | 噪音大——伤害数字之类的一次性串会灌满去重集 |

**为什么默认只认假名**：译文是简体中文，和日文汉字在字节上分不开。按「含 CJK」筛
会把已经翻好的中文台词全量记一遍，去重集瞬间撑满，真正没翻的反而被埋掉。假名中文
里不会出现，是唯一可靠的「这串没翻」标记。

`logI18nMissAll` 的正当用途是另一个问题：**确认某串走没走 native 标签**。比如战斗
结束那张图里的说话人名 `Livia Medeiros` 是拉丁字母，`logI18nMiss` 看不见它；开
`logI18nMissAll` 跑一局，日志里有它就说明走 native（补表能修），没有就说明它压根
不经过 native 标签，得去前端/服务端那层解决。
