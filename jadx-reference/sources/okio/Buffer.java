...
public final class Buffer implements BufferedSource, BufferedSink, Cloneable, ByteChannel {
    ...
    public long readLong() {
        ...
        long j2 = j | ((data[r8] & 255) << 48);
        long j3 = j2 | ((data[r1] & 255) << 40);
        long j4 = j3 | ((data[r8] & 255) << 32) | ((data[r1] & 255) << 24);
        long j5 = j4 | ((data[r6] & 255) << 16);
        long j6 = ((data[r1] & 255) << 8) | j5;
        int pos2 = pos + 1 + 1 + 1 + 1 + 1 + 1 + 1 + 1;
        long v = j6 | (data[r6] & 255);
        this.size -= 8;
        if (pos2 == limit) {
            this.head = segment.pop();
            SegmentPool.recycle(segment);
    ...
    */
        ...
        loop0: while (true) {
            ...
            if (scanOrSelect2 < 0) {
                ...
                while (triePos3 != selectLimit) {
                    if (b2 == trie[triePos3]) {
                        int nextStep = trie[triePos3 + scanOrSelect2];
                        if (pos3 != limit) {
                            pos = pos3;
                            triePos = nextStep;
                        } else {
    ...
    public void skip(long byteCount) throws EOFException {
        while (byteCount > 0) {
            ...
            int toSkip = (int) Math.min(byteCount, r0.limit - this.head.pos);
            this.size -= toSkip;
            byteCount -= toSkip;
    ...
    */
    ...
    public final ByteString md5() {
        return digest(Constants.MD5);
    }

    public final ByteString sha1() {
        return digest(Constants.SHA1);
    }
    ...
    public static final class UnsafeCursor implements Closeable {
        ...
        public final int seek(long offset) {
            ...
            if (this.segment != null) {
                long segmentOffset = this.offset - (this.start - r6.pos);
