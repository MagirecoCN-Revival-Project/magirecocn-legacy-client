...
public final class OkHostnameVerifier implements HostnameVerifier {
    ...
    public boolean verifyHostname(String hostname, String pattern) {
        if (hostname == null || hostname.length() == 0 || hostname.startsWith(TRouterMap.DOT) || hostname.endsWith("..") || pattern == null || pattern.length() == 0 || pattern.startsWith(TRouterMap.DOT) || pattern.endsWith("..")) {
            return false;
        }
        if (!hostname.endsWith(TRouterMap.DOT)) {
            hostname = hostname + '.';
        }
        if (!pattern.endsWith(TRouterMap.DOT)) {
            pattern = pattern + '.';
        }
