...
public abstract class AbstractSendChannel<E> implements SendChannel<E> {
    ...
    */
    ...
    /* JADX DEBUG: Multi-variable search result rejected for r4v1, resolved type: kotlinx.coroutines.internal.LockFreeLinkedListNode */
    /* JADX INFO: Access modifiers changed from: protected */
    /* JADX WARN: Multi-variable type inference failed */
    ...
    public final Object mo1607trySendJP2dKIU(E element) {
        Object offerInternal = offerInternal(element);
        if (offerInternal == AbstractChannelKt.OFFER_SUCCESS) {
            return ChannelResult.INSTANCE.m1627successJP2dKIU(Unit.INSTANCE);
        }
        if (offerInternal == AbstractChannelKt.OFFER_FAILED) {
            Closed<?> closedForSend = getClosedForSend();
            return closedForSend == null ? ChannelResult.INSTANCE.m1626failurePtdJZtk() : ChannelResult.INSTANCE.m1625closedJP2dKIU(helpCloseAndGetSendException(closedForSend));
        }
        if (offerInternal instanceof Closed) {
            return ChannelResult.INSTANCE.m1625closedJP2dKIU(helpCloseAndGetSendException((Closed) offerInternal));
        }
    ...
    public final void helpCloseAndResumeWithSendException(Continuation<?> continuation, E e, Closed<?> closed) {
        ...
        if (function1 != null && (callUndeliveredElementCatchingException$default = OnUndeliveredElementKt.callUndeliveredElementCatchingException$default(function1, e, null, 2, null)) != null) {
            ...
            continuation.resumeWith(Result.m116constructorimpl(ResultKt.createFailure(undeliveredElementException)));
            return;
        }
        Result.Companion companion2 = Result.INSTANCE;
        continuation.resumeWith(Result.m116constructorimpl(ResultKt.createFailure(sendException)));
    }
    ...
    private final void helpClose(Closed<?> closed) {
        ...
        while (true) {
            ...
            if (receive == null) {
                break;
            } else if (!receive.mo1651remove()) {
                receive.helpRemove();
            } else {
                m1643constructorimpl$default = InlineList.m1648plusFjFbRPM(m1643constructorimpl$default, receive);
            }
        }
        if (m1643constructorimpl$default != null) {
            if (!(m1643constructorimpl$default instanceof ArrayList)) {
                ((Receive) m1643constructorimpl$default).resumeReceiveClosed(closed);
            } else {
                Objects.requireNonNull(m1643constructorimpl$default, "null cannot be cast to non-null type java.util.ArrayList<E of kotlinx.coroutines.internal.InlineList>{ kotlin.collections.TypeAliasesKt.ArrayList<E of kotlinx.coroutines.internal.InlineList> }");
                ArrayList arrayList = (ArrayList) m1643constructorimpl$default;
