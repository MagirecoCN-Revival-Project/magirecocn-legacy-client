            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        m1655constructorimpl = SegmentOrClosed.m1655constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1655constructorimpl = SegmentOrClosed.m1655constructorimpl(segment2);
                    break;
                }
            }
            if (SegmentOrClosed.m1660isClosedimpl(m1655constructorimpl)) {
                break;
            }
            Segment m1658getSegmentimpl = SegmentOrClosed.m1658getSegmentimpl(m1655constructorimpl);
            while (true) {
                Segment segment4 = (Segment) this.tail;
                if (segment4.getId() >= m1658getSegmentimpl.getId()) {
                    break;
                }
                if (!m1658getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment4, m1658getSegmentimpl)) {
                    if (segment4.decPointers$kotlinx_coroutines_core()) {
                        segment4.remove();
                    }
                } else if (m1658getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    m1658getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m1658getSegmentimpl(m1655constructorimpl);
        ...
        do {
            ...
            if (!SegmentOrClosed.m1660isClosedimpl(m1655constructorimpl)) {
                ...
                while (true) {
                    ...
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment4, m1658getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m1658getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
