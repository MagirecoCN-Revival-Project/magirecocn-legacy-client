...
public final class zzd implements ComponentContainer {
    ...
    public zzd(Executor executor, Iterable<ComponentRegistrar> iterable, Component<?>... componentArr) {
        zzf zzfVar = new zzf(executor);
        this.zzc = zzfVar;
        ArrayList arrayList = new ArrayList();
        arrayList.add(Component.of(zzfVar, zzf.class, Subscriber.class, Publisher.class));
        Iterator<ComponentRegistrar> it = iterable.iterator();
        while (it.hasNext()) {
            arrayList.addAll(it.next().getComponents());
        }
        Collections.addAll(arrayList, componentArr);
        List<Component<?>> unmodifiableList = Collections.unmodifiableList(zze.zza(arrayList));
        this.zza = unmodifiableList;
        Iterator<Component<?>> it2 = unmodifiableList.iterator();
        while (it2.hasNext()) {
            zza(it2.next());
        }
        zza();
